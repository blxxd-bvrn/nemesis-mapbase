...
#include "npc_vortigaunt_enemy.h"
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_Vortigaunt_Enemy)
...
LINK_ENTITY_TO_CLASS( npc_vortigaunt_enemy, CNPC_Vortigaunt_Enemy);

IMPLEMENT_SERVERCLASS_ST(CNPC_Vortigaunt_Enemy, DT_NPC_Vortigaunt )
...
// Purpose: 
//-----------------------------------------------------------------------------
CNPC_Vortigaunt_Enemy::CNPC_Vortigaunt_Enemy( void ) :
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_Enemy::PlayerBelowHealthPercentage( CBasePlayer *pPlayer, float flPerc )
{
	if ( pPlayer == NULL )
		return false;
...
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::SpeakSentence( int sentenceType )
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask)
	{
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_RANGE_ATTACK1:
		...
		//Gonna change it for player jic
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::AlertSound( void )
{
...
//-----------------------------------------------------------------------------
float CNPC_Vortigaunt_Enemy::MaxYawSpeed ( void )
{
	switch ( GetActivity() )
	{
	case ACT_IDLE:		
...
// Output : 
//-----------------------------------------------------------------------------
Vector  CNPC_Vortigaunt_Enemy::FacingPosition( void )
{
...
Vector CNPC_Vortigaunt_Enemy::BodyTarget( const Vector &posSrc, bool bNoisy )
{ 
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_Enemy::InnateWeaponLOSCondition( const Vector &ownerPos, const Vector &targetPos, bool bSetConditions )
{
...
//------------------------------------------------------------------------------
int CNPC_Vortigaunt_Enemy::RangeAttack1Conditions( float flDot, float flDist )
{
	if ( GetEnemy() == NULL )
		return COND_NONE;
...
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt_Enemy::MeleeAttack1Conditions( float flDot, float flDist )
{
...
// Output : int
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt_Enemy::NumAntlionsInRadius( float flRadius )
{
...
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt_Enemy::RangeAttack2Conditions( float flDot, float flDist )
{
	return COND_NONE;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::HandleAnimEvent( animevent_t *pEvent )
{
...
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::InputTurnBlue( inputdata_t &data )
{
...
// Purpose : Turn blue or green
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::InputTurnBlack( inputdata_t &data )
{
...
//------------------------------------------------------------------------------
Activity CNPC_Vortigaunt_Enemy::NPC_TranslateActivity( Activity eNewActivity )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::UpdateOnRemove( void)
{
	ClearBeams();
	ClearHandGlow();
...
// Purpose :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::Event_Killed( const CTakeDamageInfo &info )
{
	ClearBeams();
	ClearHandGlow();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::Spawn( void )
{
	...
	SetUse( &CNPC_Vortigaunt_Enemy::Use );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::Precache()
{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
// PainSound
//=========================================================
void CNPC_Vortigaunt_Enemy::PainSound( const CTakeDamageInfo &info )
{
...
// DeathSound 
//=========================================================
void CNPC_Vortigaunt_Enemy::DeathSound( const CTakeDamageInfo &info )
{
	Speak( VORT_DIE );
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
...
// Purpose:
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt_Enemy::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::SetHealTarget( CBaseEntity *pTarget, bool bPlayerRequested )
{
...
//-----------------------------------------------------------------------------
CBaseEntity * CNPC_Vortigaunt_Enemy::FindHealTarget( void )
{
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_Enemy::HealBehaviorAvailable( void )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_Enemy::ShouldHealTarget( CBaseEntity *pTarget )
{
...
// Output : int
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt_Enemy::SelectHealSchedule( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::ClearSchedule( const char *szReason )
{
	MaintainGlows();
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::OnScheduleChange( void )
{
...
//------------------------------------------------------------------------------
int CNPC_Vortigaunt_Enemy::SelectSchedule( void )
{
...
// 
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt_Enemy::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::DeclineFollowing( void )
{
	Speak( VORT_POK );
}
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_Enemy::CanBeUsedAsAFriend( void )
{
...
bool CNPC_Vortigaunt_Enemy::FInViewCone( CBaseEntity *pEntity )
{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::StartHealing( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::StopHealing( bool bInterrupt )
{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::MaintainHealSchedule( void )
{
	// Need to be healing
	if ( m_eHealState == HEAL_STATE_NONE )
		return;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
inline bool CNPC_Vortigaunt_Enemy::InAttackSequence( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::MaintainGlows( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::OnRestore( void )
{
	BaseClass::OnRestore();
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::PrescheduleThink( void )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_Enemy::OverrideMoveFacing( const AILocalMoveGoal_t &move, float flInterval )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::BuildScheduleTestBits( void )
{
	// Call to base
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::ArmBeam( int beamType, int nHand )
{
	trace_t tr;
	float flDist = 1.0;
...
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::StartHandGlow( int beamType, int nHand )
{
...
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::EndHandGlow( int beamType /*= VORTIGAUNT_BEAM_ALL*/ )
{
	if ( m_hHandEffect[0] )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_Enemy::IsValidEnemy( CBaseEntity *pEnemy )
{
	if ( IsRoller( pEnemy ) )
	{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::CreateBeamBlast( const Vector &vecOrigin )
{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::ZapBeam( int nHand )
{
	Vector forward;
...
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::ClearHandGlow( void )
{
...
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::ClearBeams( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::InputEnableArmorRecharge( inputdata_t &data )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::InputDisableArmorRecharge( inputdata_t &data )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::InputChargeTarget( inputdata_t &data )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::InputExtractBugbait( inputdata_t &data )
{
	CBaseEntity *pTarget = gEntList.FindEntityByName( NULL, data.value.String(), NULL, data.pActivator, data.pCaller );
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::InputEnableHealthRegeneration( inputdata_t &data )
{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::InputDisableHealthRegeneration( inputdata_t &data )
{
...
// Purpose:
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt_Enemy::IRelationPriority( CBaseEntity *pTarget )
{
...
//-----------------------------------------------------------------------------
Disposition_t CNPC_Vortigaunt_Enemy::IRelationType( CBaseEntity *pTarget )
{
	if ( pTarget == NULL )
		return D_NU;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_Enemy::HealGestureHasLOS( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::GatherHealConditions( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::GatherConditions( void )
{
	// Call our base
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::DispelAntlions( const Vector &vecOrigin, float flRadius, bool bDispel /*= true*/ )
{
	// More effects
	if ( bDispel )
	{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::InputDispel( inputdata_t &data )
{
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_Enemy::CanRunAScriptedNPCInteraction( bool bForced /*= false*/ )
{
...
// Input  : interrupt - 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::SetScriptedScheduleIgnoreConditions( Interruptability_t interrupt )
{
...
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt_Enemy::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_Enemy::ShouldMoveAndShoot( void )
{
...
// line.
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::OnSquishedGrub( const CBaseEntity *pGrub )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::AimGun( void )
{
#ifdef MAPBASE
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::OnStartScene( void )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_Enemy::IsInterruptable( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::InputBeginCarryNPC( inputdata_t &indputdata )
{
	m_bCarryingNPC = true;
}
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::InputEndCarryNPC( inputdata_t &indputdata )
{
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_Enemy::CanFlinch( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_Enemy::OnUpdateShotRegulator( void )
{
...
AI_BEGIN_CUSTOM_NPC( npc_vortigaunt, CNPC_Vortigaunt_Enemy)
